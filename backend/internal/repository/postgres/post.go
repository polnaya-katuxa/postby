...
func (p PostRepository) GetAll(ctx context.Context, uuid uuid.UUID) ([]*logicModels.Post, error) {
	...
	res := p.db.WithContext(ctx).Table("posts").Where("writer_id = ?", uuid).Order("public_time desc").Find(&postsDB)
	if errors.Is(res.Error, gorm.ErrRecordNotFound) || res.RowsAffected == 0 {
		return nil, nil
	}
	if res.Error != nil {
...
func (p PostRepository) GetByIDAndSpan(ctx context.Context, uuids []uuid.UUID, time1 time.Time, time2 time.Time, isIn bool) ([]*logicModels.Post, error) {
	...
	if isIn {
		res = p.db.WithContext(ctx).Table("posts").Where("public_time BETWEEN ? AND ? AND writer_id IN ?", time1, time2, uuids).Order("public_time desc").Find(&postsDB)
		if errors.Is(res.Error, gorm.ErrRecordNotFound) || res.RowsAffected == 0 {
			return nil, nil
		}
		if res.Error != nil {
		...
		if len(uuids) == 0 {
			res = p.db.WithContext(ctx).Table("posts").Where("public_time BETWEEN ? AND ? AND perms = 'free'", time1, time2).Order("public_time desc").Find(&postsDB)
			if errors.Is(res.Error, gorm.ErrRecordNotFound) || res.RowsAffected == 0 {
				return nil, nil
			}
			if res.Error != nil {
			...
			res = p.db.WithContext(ctx).Table("posts").Where("public_time BETWEEN ? AND ? AND writer_id NOT IN ? AND perms = 'free'", time1, time2, uuids).Order("public_time desc").Find(&postsDB)
			if errors.Is(res.Error, gorm.ErrRecordNotFound) || res.RowsAffected == 0 {
				return nil, nil
			}
			if res.Error != nil {
