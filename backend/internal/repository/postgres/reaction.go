...
func (r ReactionRepository) Reacted(ctx context.Context, uuid1 uuid.UUID, uuid2 uuid.UUID) (bool, error) {
	...
	res := r.db.WithContext(ctx).Table("reactions").Where("reactor_id = ? AND post_id = ?", uuid1, uuid2).Take(&react)
	if errors.Is(res.Error, gorm.ErrRecordNotFound) || res.RowsAffected == 0 {
		return false, nil
	}
	if res.Error != nil {
		return false, fmt.Errorf("select: %w", res.Error)
	}
...
func (r ReactionRepository) Delete(ctx context.Context, uuid1 uuid.UUID, uuid2 uuid.UUID) error {
	res := r.db.WithContext(ctx).Table("reactions").Where("reactor_id = ? AND post_id = ?", uuid1, uuid2).Delete(&repoModels.Reaction{})
	if res.Error != nil {
