...
func (m *Menu) viewProfileHandler(opt wmenu.Opt) error {
	...
	posts, err := m.postLogic.GetAll(ctx, user.UUID)
	if err != nil {
...
func (m *Menu) viewFeedHandler(opt wmenu.Opt) error {
	...
	sPosts, nsPosts, err := m.feedLogic.View(ctx)
	if err != nil {
		return fmt.Errorf("view: %w", err)
	}

	printPosts(sPosts, "SUB POSTS")
	printPosts(nsPosts, "NO SUB POSTS")
...
func (m *Menu) viewCommentsHandler(opt wmenu.Opt) error {
	...
	comms, err := m.postLogic.GetAllComments(ctx, postID)
	if err != nil {
...
func (m *Menu) viewUsersHandler(opt wmenu.Opt) error {
	...
	users, err := m.profileLogic.GetAll(ctx)
	if err != nil {
		return fmt.Errorf("get all: %w", err)
	}
