...
func NewPL(r interfaces.IReactionRepository, bt interfaces.IBalanceTransactionLogic, p interfaces.IPostRepository,
	s interfaces.ISubscriptionLogic, c interfaces.ICommentRepository) *Post {
	return &Post{
		reactionRepo:            r,
...
func (p *Post) GetAll(ctx context.Context, userID uuid.UUID) ([]*models.Post, error) {
	user, err := mycontext.UserFromContext(ctx)
	if err != nil {
	...
	posts, err := p.postRepo.GetAll(ctx, userID)
	if err != nil {
...
func (p *Post) React(ctx context.Context, postID uuid.UUID, typeID uuid.UUID) (bool, error) {
	...
	ok, err = p.reactionRepo.Reacted(ctx, user.UUID, post.UUID)
	if err != nil {
	...
	if ok {
		err = p.reactionRepo.Delete(ctx, user.UUID, post.UUID)
		if err != nil {
...
func (p *Post) GetAllComments(ctx context.Context, postID uuid.UUID) ([]*models.Comment, error) {
	...
	c, err := p.commentRepo.GetAll(ctx, postID)
	if err != nil {
...
func (p *Post) PopularityCheck(ctx context.Context, userID uuid.UUID) error {
	...
	posts, err := p.postRepo.GetAll(ctx, userID)
	if err != nil {
		mycontext.LoggerFromContext(ctx).Warnw("cannot get user's posts", "user", userID, "post", "error", err)
